...
remote_stop() {
  ...
  docker ps --filter "name=zeroone-" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
}

remote_status() {
  docker ps --filter 'name=zeroone-' --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'
}

main_deploy() {
  bundle_images ""

  docker network inspect zeroone-net >/dev/null 2>&1 || docker network create zeroone-net
