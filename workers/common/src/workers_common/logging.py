...
# ---------------------------------------------------------------------------
# request_id 上下文（相对导入 _context，不经过 workers_common/__init__.py）
# ---------------------------------------------------------------------------
...
def configure_logging(
    ...
    # 清空旧 handlers，重建，避免 celery 等框架重置后重复
    root.handlers.clear()
...
def reinit_file_logging(
    service_name: Optional[str] = None,
    log_dir: Optional[str] = None,
    log_file_max_bytes: Optional[int] = None,
    log_file_backup_count: Optional[int] = None,
) -> bool:
    """在 fork 后的子进程中重建文件日志写入线程。

    背景：
      multiprocessing fork 只复制内存，不复制线程。
      父进程的 QueueListener 线程在子进程里不存在，但 _state.file_queue
      仍为非 None（内存复制），导致日志被投入队列却无人消费。

    本函数：
      1. 停止旧的 listener（如果存在且尚未停止）
      2. 重置 file_queue / queue_listener 为 None
      3. 用新参数重建 RotatingFileHandler + QueueListener

    若 log_file 为 False 或 log_dir 无法解析，则不重建，返回 False。

    Args:
        service_name        : 日志文件名前缀，None 则沿用现有 _state
        log_dir             : 日志目录，None 则不重新解析（沿用上次）
        log_file_max_bytes  : 单文件最大字节，None 沿用现有
        log_file_backup_count: 保留份数，None 沿用现有

    Returns:
        True 表示文件日志线程已重建，False 表示未启用
    """
    if not _state.log_file:
        return False

    # 停止残留 listener（fork 后对象在但线程不在，stop 是幂等的）
    ...
    # 重置队列状态，使 _init_file_writer 的幂等检查放行
    _state.file_queue = None
