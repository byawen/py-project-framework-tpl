...
from pingpong_worker.app.infrastructure.persistence.models.pong_model import PongModel
...
class SQLPongRepository(PongRepository):
    ...
    async def get_by_id(self, pong_id: str) -> Optional[Pong]:
        ...
        async with self.dm.session() as session:
            result = await session.execute(select(PongModel).where(PongModel.pong_id == pong_id))
