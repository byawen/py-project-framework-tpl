...
class OtherServiceAPIProxy(UserService):
    ...
    def __init__(self, setting: Settings):
        ...
        if setting.MODEL == "all-in-one":
            from pingpong_worker.clients.other_service.local_api import LocalUserService

            self.proxy = LocalUserService()
        else:
            from pingpong_worker.clients.other_service.remote_api import RemoteUserService

            self.proxy = RemoteUserService(
                base_url=setting.OTHER_SERVICE_URL or "http://127.0.0.1:8001"
            )
